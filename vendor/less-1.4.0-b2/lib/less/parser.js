    ...
    return parser = {
        ...
        parsers: {
            ...
            nthchild: function () {
                ...
                if ($("(")) {
                    var keyword, multiplierOrAddition, operator, addition;

                    if (keyword = $(/^(odd|even)(?:\))/)) {
                        return new(tree.NthChild)(keyword[1]);
                    }

                    if (multiplierOrAddition = $(this.entities.variableCurly) || $(/[-+]?\d+/)) {
                        if ($(")")) {
                            return new(tree.NthChild)(multiplierOrAddition);
                        }
                        if ($("n")) {
                            addition = $(this.entities.variableCurly);
                            if (!addition) {
                                operator = $(/[-+]/);
                                addition = $(this.entities.variableCurly) || $(/\d+/);
                            }
                            if ($(")")) {
                                return new(tree.NthChild)(multiplierOrAddition, operator, addition);
                            }
                        }
                    }
                }
